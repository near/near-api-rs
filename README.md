...
* `testing`: Enables integration with [`near-sandbox-utils`](https://github.com/near/near-sandbox-utils-rs) for testing purposes. This feature allows you to launch a local sandbox instance of NEAR network and use it for testing and development. This feature is developed as a replacement for the `near-workspaces` as it doesn't make sense to have `near-api-rs` and `near-workspaces` together. The feature is in the early stage of development and not all the sandbox features are supported yet.
