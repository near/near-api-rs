...
#[tokio::test]
async fn test_that_generic_account_has_no_storage() -> TestResult {
...
#[tokio::test]
async fn test_deposit_integration() -> TestResult {
    ...
    assert_eq!(balance.data.ok_or("Balance is none")?.total, deposit_amount);

    Ok(())
}

#[tokio::test]
async fn test_withdraw_integration() -> TestResult {
    let ctx = setup_social_contract().await?;
    ...
    let initial_available = initial_balance
        .data
        .as_ref()
    ...
    assert_eq!(
        balance.data.ok_or("Balance is none")?.total,
...
#[tokio::test]
async fn test_unregister_integration() -> TestResult {
...
#[tokio::test]
async fn test_registration_only_integration() -> TestResult {
    let ctx = setup_social_contract().await?;
    ...
    assert!(
        balance.data.as_ref().ok_or("Balance is none")?.total < deposit_amount,
        "Should have refunded the excess deposit, but got {}",
        balance.data.as_ref().ok_or("Balance is none")?.total
    );
...
#[tokio::test]
async fn test_force_unregister_integration() -> TestResult {
    let ctx = setup_ft_contract().await?;
